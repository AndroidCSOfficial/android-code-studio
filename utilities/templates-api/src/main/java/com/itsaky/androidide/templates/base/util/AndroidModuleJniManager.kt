...
class AndroidModuleJniManager {
  ...
  fun AndroidModuleTemplateBuilder.jniDir(srcSet: SrcSet = Main): File {
    return File(File(moduleDir, "src/${srcSet.name}"), "cpp").also { it.mkdirs() }
  }
  ...
  /**
   * Converts a Java package name to JNI function prefix.
   * Example: "com.example.myapp" -> "Java_com_example_myapp"
   *
